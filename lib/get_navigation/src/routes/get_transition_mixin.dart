...
/// Mixin for widgets that should block back gesture.
///
/// 用于标记组件，表示该组件**应该**拦截侧滑返回手势。
/// 当手势触发区域包含使用此 mixin 的组件时，侧滑返回将被禁用。
///
/// Example:
/// ```dart
/// class MyCustomGestureArea extends StatelessWidget with BlockBackGesture {
///   @override
///   Widget build(BuildContext context) {
...
/// }
/// ```
mixin BlockBackGesture on Widget {}
...
const double _kMinFlingVelocity = 5.0; // Milliseconds.
...
class CupertinoBackGestureDetectorState<T> extends State<CupertinoBackGestureDetector<T>> {
  ...
  late MyHorizontalDragGestureRecognizer _recognizer;
  ...
  void initState() {
    super.initState();
    _recognizer = MyHorizontalDragGestureRecognizer(debugOwner: this)
  ...
  bool _isPointerOverHorizontalScrollable(PointerDownEvent event) {
    ...
    for (final entry in result.path) {
      ...
      if (element != null) {
        ...
        // 检查当前 Widget 是否使用了 BlockBackGesture，需要拦截
        if (_hasBlockBackGesture(element)) {
          // print('  -> 该组件使用了 BlockBackGesture，拦截侧滑');
          return true;
        }
  ...
  // 检查 Element 或其祖先是否使用了 BlockBackGesture mixin
  bool _hasBlockBackGesture(Element element) {
    // 检查当前 Widget
    if (element.widget is BlockBackGesture) {
      return true;
    }

    // 向上查找祖先中是否有使用 BlockBackGesture 的 Widget
    bool found = false;
    element.visitAncestorElements((ancestor) {
      if (ancestor.widget is BlockBackGesture) {
        found = true;
        return false; // 停止遍历
      }
      // 只向上查找 5 层，避免过度遍历
        ...
        default:
          if (Get.customTransition != null) {
            return Get.customTransition!
                .buildTransition(context, route.curve, route.alignment, animation, secondaryAnimation, child);
          }
